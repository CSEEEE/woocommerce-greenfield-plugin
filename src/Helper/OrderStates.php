...
class OrderStates {
	...
	public function getOrderStateLabels(): array {
		return [
			self::NEW => _x('New', 'global_settings', 'zeuspay-for-woocommerce'),
			self::PROCESSING => _x('Paid', 'global_settings', 'zeuspay-for-woocommerce'),
			self::SETTLED => _x('Settled', 'global_settings', 'zeuspay-for-woocommerce'),
			self::SETTLED_PAID_OVER => _x('Settled (paid over)', 'global_settings', 'zeuspay-for-woocommerce'),
			self::INVALID => _x('Invalid', 'global_settings', 'zeuspay-for-woocommerce'),
			self::EXPIRED => _x('Expired', 'global_settings', 'zeuspay-for-woocommerce'),
			self::EXPIRED_PAID_PARTIAL => _x('Expired with partial payment', 'global_settings', 'zeuspay-for-woocommerce')
		];
	}
	...
	public function renderOrderStatesHtml($value) {
		...
		$wcStates = [self::IGNORE => _x('- no mapping / defaults -', 'global_settings', 'zeuspay-for-woocommerce')] + $wcStates;
		...
		<tr valign="top">
			...
			<td class="forminp" id="<?php echo esc_attr($value['id']) ?>">
				...
				<p class="description">
					<?php echo _x( 'By keeping default behavior for the "Settled" status you make sure that WooCommerce handles orders of virtual and downloadable products only properly and set those orders to "complete" instead of "processing" like for orders containing physical products.', 'global_settings', 'zeuspay-for-woocommerce' ); ?>
				</p>
			</td>
		</tr>
		<?php
